...
bool Shader::LoadFromStrings(std::string vertexString, std::string fragmentString)
{
    ...
    if(!success)
    {
        ...
        Logger::log(Logger::Level::Error, "Failed to compile vertex shader: {}", infoLog);
        return false;
    }
    ...
    if(!success)
    {
        ...
        Logger::log(Logger::Level::Error, "Failed to compile fragment shader: {}", infoLog);
        return false;
    }
    ...
    if(!success) {
        ...
        Logger::log(Logger::Level::Error, "Failed to link shader program: {}", infoLog);
        return false;
    }
...
bool Shader::LoadFromFile(std::string vertexFilePath, std::string fragmentFilePath)
{
    ...
    if (myInFile.fail())
    {
        Logger::log(Logger::Level::Error, "Failed to open vertex shader file {}.", vertexFilePath.c_str());
    }
    ...
    if (myInFile.fail())
    {
        Logger::log(Logger::Level::Error, "Failed to open fragment shader file {}.", fragmentFilePath.c_str());
        return false;
    }
